...
char *test_fvn1a() {
    uint32_t hash = Hashmap_fvn1a_hash(&test1);
    mu_assert(hash != 0, "Bad hash for test1");

    hash = Hashmap_fvn1a_hash(&test2);
    mu_assert(hash != 0, "Bad hash for test2");

    hash = Hashmap_fvn1a_hash(&test3);
    mu_assert(hash != 0, "Bad hash for test3");
...
#define NUM_KEYS BUKCETS * 1000
enum {ALGO_FVN1A, ALGO_ADLER32, ALGO_DJB};
