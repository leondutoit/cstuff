...
char *test_push_pop() {
    ...
    for (i = 0; i < NUM_TESTS; i++) {
        ...
        printf("%s\n", tests[i]);
        mu_assert(Stack_peek(stack) == tests[i], "wrong next val");
    }

    mu_assert(Stack_count(stack) == NUM_TESTS, "wrong count in stack");
...
char *all_tests() {
    ...
    //mu_run_test(test_destroy); segfault dunno why
