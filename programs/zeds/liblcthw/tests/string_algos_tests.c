...
char *test_find_and_scan() {
    ...
    mu_assert(find_i > 0, "failed to find pattern ALPHA in string");

    int scan_i = StringScanner_scan(scan, &ALPHA);
    mu_assert(scan_i > 0, "failed to find pattern ALPHA with scan");
...
char *test_binstr_performance() {
    ...
    time_t elapsed = 0; // defined for storing system time values
    ...
    debug("BINSTR_COUNT: %lu, END_TIME: %d, OPS: %f",
...
char *test_find_performance() {
    ...
    debug("FIND_COUNT: %lu, END_TIME: %d, OPS: %f",
        find_count, (int)elapsed, (double)find_count / elapsed);
...
char *test_scan_performance() {
    ...
    debug("SCAN_COUNT: %lu, END_TIME: %d, OPS: %f",
        find_count, (int)elapsed, (double)find_count / elapsed);
