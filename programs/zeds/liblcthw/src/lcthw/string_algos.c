...
// Boyer-Moore-Horspool (BMH) string search algorithm
// http://en.wikipedia.org/wiki/Boyer%E2%80%93Moore%E2%80%93Horspool_algorithm

#include <lcthw/dbg.h>
#include <lcthw/string_algos.h>
#include <limits.h> // what are this?

static inline void String_setup_skip_chars(
        size_t *skip_chars,
        const unsigned char *needle, // what to look for
        ssize_t nlen) {

    size_t i = 0;
    size_t last = nlen - 1; // length to last char in pattern

    // initialise the entire skip_chars array with nlen
    for (i = 0; i < UCHAR_MAX + 1; i++) {
        skip_chars[i] = nlen;
    }
    // index into each char in the needle
    // use this to index into the skip_chars list
    // set that element to last - i (last - i == how far to end of pattern)
    // call this the ditance to end of pattern, or distance
    // put distance into skip_chars
    for (i = 0; i < last; i++) {
...
static inline const unsigned char *String_base_search(
    ...
    while (hlen > nlen) { // while the pattern can still be found
        // go backwards from last until h elem equal to n elem
        for (i = last; haystack[i] == needle[i]; i--) {
            // and until we've looked through the entire thing
            if (i == 0) {
                return haystack;
            }
        }
        hlen -= skip_chars[haystack[last]]; // decrement hlen
        haystack += skip_chars[haystack[last]]; // build the haystack
    }
...
StringScanner *StringScanner_create(bstring in) {
    ...
    assert(scan != NULL && "scan was NULL");
    return scan;
...
static inline void StringScanner_set_needle(StringScanner *scan, bstring tofind) {
    ...
    String_setup_skip_chars(scan->skip_char, scan->needle, scan->nlen);
}
