...
int check_function(const char *func_to_run, const char *data, int expected) {
    ...
    check(func != NULL, "Did not find func %s in lib %s: %s",
        func_to_run, lib_file, dlerror());

    int rc = func(data);
    check(rc == expected, "Func %s return %d for data: %s",
        func_to_run, rc, data);
...
char *test_failures() {
    mu_assert(check_function("fail_on_purpose", "Hello", 1), "fail_on_purpose shoulf fail.");
